window.onload = function()
{
    ...
    */
    ...
    var decimalFraccionario = function()
    {
        ...
        */
        var aux= Math.pow(10, String(datosDecimal.decimal).length);

        var num= (datosDecimal.entero+"."+datosDecimal.decimal)*aux;
        
       var dem=1*aux;

       var resuelve=num+"/"+dem;

        console.log(datosDecimal, aux, num, dem );
        //Imprime la respuesta en la vista (HTML)...
        nom_div("resFraccionario").innerHTML = simplificar(resuelve);//simplificar(resuelve);
        
    };
var simplificar=function(val)
{
    ...
    var binarioDecimal = function()
    {
        ...
        */
        var f;
          ...
          for (var i=8; i>=1; i--)
          {
              f = Number(nom_div("valor_"+i).value);
              console.log(f);
              if (f===1){
                res += Math.pow(2, exp); 
